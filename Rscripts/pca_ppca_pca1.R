...
fstats_pca <- function(X3,method,npcs){
  ...
  data = data/2
    ...
    if (method=='pca'){
      ...
      xxx = PCA1(as.matrix(data), n_pcs=npcs, normalized=TRUE)
    }else if (method=='ppca_cor'){
      xxx = ppca_cor(as.matrix(data), n_pcs=npcs, normalized=TRUE)
    }
...
plotf <- function(outf){
  ...
  par(mfrow=c(1,4))
  plot(PC, eigenvalues, main="PCA", ylim=c(0,0.7))
  plot(PC, eigenvalues1, main="Probabilistic PCA", ylim=c(0,0.7))
  plot(PC, eigenvalues2, main="PCA with binomial error", ylim=c(0,0.7))
  plot(PC, eigenvalues3, main="PPCA with binomial error", ylim=c(0,0.7))
  dev.off()
