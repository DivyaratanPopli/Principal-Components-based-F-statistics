...
Because the amount of preserved DNA is often a limiting factor of ancient DNA data, low-coverage and of heterogeneous data quality arr common issues in data analysis  \cite{orlando_ancient_2021}. In this case, missingness can make individual-based statistics even harder to estimate.
...
The three F-statistics are defined in terms of population allele frequencies as follows:
...
Here, we denote the observed sample allele frequency as $x$. This sampling bias affects the estimates of $F_3$ as well but the correction terms cancel out for $F_4$, where the two estimators coincide.
...
Oteo-Garcia \& Oteo \cite{oteo-garcia_geometrical_2021} showed that since $F_2$ is just a squared Euclidean distance, we can think of F-statistics in terms of Euclidean distances in an allele frequency space. In this framework, each population can be represented as a point in a multi dimensional allele frequency space ($\mathcal{X}_1 = (\mathcal{X}_{11}, \dots, \mathcal{X}_{1S})$, and $F_2(X_1, X_4)$ is then the  squared Euclidean distance between these points (Fig. \ref{fig2:overview} E). $F_3(X_1;X_3,X_4)$ is then a dot product of the vectors $\Vec{x1} - \Vec{x3}$ and $\Vec{x1} - \Vec{x4}$ (Fig. \ref{fig2:overview} F). $F_4(X_1,X_3;X_2,X_4)$ is a dot product of vectors $\Vec{x1} - \Vec{x3}$ and $\Vec{x2} - \Vec{x4}$ (Fig. \ref{fig2:overview} G), and $F_4(X_1,X_2;X_3,X_4)$ is a dot product of vectors $\Vec{x1} - \Vec{x2}$ and $\Vec{x3} - \Vec{x4}$ (Fig. \ref{fig2:overview} H). 

\begin{align}\label{eq:f_geometric}
F_2(X_1,X_4) &= \frac{1}{S}||\Vec{\mathcal{X}_{1}} - \vec{\mathcal{X}_{4}}||^2\nonumber\\
F_3(X_1;X_3,X_4) &= \frac{1}{S}||\vec{\mathcal{X}_{1}} - \vec{\mathcal{X}_{3}})\cdot(\vec{\mathcal{X}_{1}} - \vec{\mathcal{X}_{4}})||\nonumber\\
F_4(X_1,X_2;X_3,X_4) &= \frac{1}{S}||(\mathcal{X}_{1} - \mathcal{X}_{2})\cdot(\mathcal{X}_{3} - \mathcal{X}_{4})||\nonumber\\
\end{align}
...
Let us assume M populations and S SNPs, such that our dataset $\mathbf{X}$ has the dimension $[M \times S]$, and each entry of $\mathbf{X}$ is an allele frequency $\in$ [0,1]. PCA of mean-centered X allows us to project this $[M \times S]$ high dimensional data on a lower dimensional subspace $[M \times q]$. $q = M-1$ represents the case where we retain all the PCs, and thus PCA only rotates X. However, in practice we often only need few PCs (q << M) to explain most variation in the populations \cite{peter_geometric_2022}. A common algorithm to estimate PCs is Singular Value Decomposition (SVD). For this approach we first mean-center $\mathbf{X}$, and then decompose $\mathbf{X}$ into $\mathbf{U}$, $\mathbf{\Sigma}$, and $\mathbf{V^T}$.
...
The covariance used by LSE is an estimated covariance matrix, and its properties differ from the sample covariance matrix used in classical PCA. In particular, sample covariance matrices are positive semi-definite, which means that all eigenvalues are non-negative. In contrast, for an estimated covariance matrix, the expecation of the smallest $M-q$ eigenvalues is zero, and thus an unbiased estimate will have both positive and negative eigenvalues. PCs with negative eigenvalues correspond to imaginary PCs, and thus, we need to adjust eq. \ref{eq:f_pca} as
